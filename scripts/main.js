...
const selectedDepartment = departments[userInput1];

console.log(`${userInput1} is een leuke afdeling om te werken. Er werken op dit moment ${selectedDepartment.numberOfEmployees} medewerkers.`);

let jobPrompt = `Je koos ${userInput1}. Over welke functie wil je meer weten? Voer een getal tussen 0 en ${selectedDepartment.jobs.length - 1} in.\n`;

selectedDepartment.jobs.forEach((job, index) => {
    jobPrompt += `${index}: ${job.title}\n`;
});

const selectedJobIndex = parseInt(prompt(jobPrompt));

if (!isNaN(selectedJobIndex) && selectedJobIndex >= 0 && selectedJobIndex < selectedDepartment.jobs.length) {
    const selectedJob = selectedDepartment.jobs[selectedJobIndex];
